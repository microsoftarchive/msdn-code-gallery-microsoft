...
--*/
{
    ...
    TracePrint((TRACE_LEVEL_INFORMATION,
                TRACE_FLAG_IOCTL,
                "GetLBAStatus command issued (consolidateblocks only = %!bool!. NT Status: %!STATUS!.\n",
...
--*/
{
    ...
    *BlockLimitsDataMayHaveChanged = FALSE;
    ...
    //
    // Calculate the maximum number of slabs that could be returned by Get LBA
    // Status.  The max buffer size could either be capped by the Parameter
    // Data Length field or the Max. Transfer Length of the adapter.
    //
...
--*/
{
    ...
    if (pageData) {
        ...
        if (pageData->QERR != 0) {
            ...
            status = ClassModeSelect(DeviceObject,
                                     (PCHAR)modeData,
                                     size,
                                     pageSavable);
