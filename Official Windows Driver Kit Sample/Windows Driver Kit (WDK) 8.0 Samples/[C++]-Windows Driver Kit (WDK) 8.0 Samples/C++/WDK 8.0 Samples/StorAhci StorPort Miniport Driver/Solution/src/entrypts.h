...
typedef struct _ATA_SUPPORTED_GPL_PAGES {
    ...
    struct {
        ULONG  LogAddressSupported : 1;     // Log Address 0x10
        ULONG  Reserved : 31;                   // no other log pages available
    } NcqCommandError;

    struct {
        ULONG  LogAddressSupported : 1;     // Log Address 0x24
        ULONG  Reserved : 31;                   // no other log pages available
    } CurrentDeviceInternalStatusData;

    struct {
        ULONG  LogAddressSupported : 1;     // Log Address 0x25
        ULONG  Reserved : 31;                   // no other log pages available
    } SavedDeviceInternalStatusData;
...
typedef struct _AHCI_SRB_EXTENSION {
    ...
    union {
        ...
        NVCACHE_HINT_PAYLOAD    HintPayload;    // contains part of info related to Host to Device FIS (0x27) 
    };
