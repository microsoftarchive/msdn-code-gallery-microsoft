...
--*/
{
  ...
  //1.1 Map SRB fields to CFIS fields
    ...
    if ( (ChannelExtension->StateFlags.NCQ_HybridInfo == 1) && IsNCQWriteCommand(srbExtension) ) {
...
--*/
{
    PAHCI_SRB_EXTENSION     srbExtension = GetSrbExtension(SlotContent->Srb);
    PAHCI_COMMAND_TABLE     cmdTable = (PAHCI_COMMAND_TABLE)srbExtension;
    PNVCACHE_HINT_PAYLOAD   hintPayload = &srbExtension->HintPayload;
    ...
    command = hintPayload->Command;

    if (command == IDE_COMMAND_NCQ_NON_DATA) {
        subCommand = (hintPayload->Feature7_0 & 0x0F);      // subCommand is the lower 4 bits in Feature7_0 field.
        NT_ASSERT( (subCommand == IDE_NCQ_NON_DATA_HYBRID_CHANGE_BY_SIZE) ||
        ...
        subCommand = (hintPayload->Count15_8 & 0x1F);       // subCommand is the lower 5 bits in Count15_8 field.
        NT_ASSERT(subCommand == IDE_NCQ_SEND_HYBRID_EVICT);
    } else {
        subCommand = IDE_COMMAND_NOT_VALID;
        NT_ASSERT(subCommand != IDE_COMMAND_NOT_VALID);
    }
  ...
  //1.1 Map NVCACHE_HINT_PAYLOAD fields to CFIS fields
    ...
    cmdTable->CFIS.Reserved1 = 0;
    cmdTable->CFIS.C = 1;
    cmdTable->CFIS.Command = hintPayload->Command;

    cmdTable->CFIS.Device = (1 << 6);
    cmdTable->CFIS.Count7_0 = (srbExtension->QueueTag << 3);

    if ((command == IDE_COMMAND_NCQ_NON_DATA) && (subCommand == IDE_NCQ_NON_DATA_HYBRID_CONTROL)) {
        cmdTable->CFIS.Feature7_0 = (hintPayload->Feature7_0 & 0x8F);   //bit 4,5,6 are reserved.
        cmdTable->CFIS.Feature15_8 = 0;                                 //reserved field for this subcommand
    } else {
        cmdTable->CFIS.Feature7_0 = hintPayload->Feature7_0;
        cmdTable->CFIS.Feature15_8 = hintPayload->Feature15_8;
    }

    if ((command == IDE_COMMAND_NCQ_NON_DATA) && (subCommand == IDE_NCQ_NON_DATA_HYBRID_CONTROL)) {
        cmdTable->CFIS.Count15_8 = 0;                                   //reserved field for this subcommand
    } else if ((command == IDE_COMMAND_SEND_FPDMA_QUEUED) && (subCommand == IDE_NCQ_SEND_HYBRID_EVICT)) {
        cmdTable->CFIS.Count15_8 = (hintPayload->Count15_8 & 0x1F);     //StorAHCI doesn't support PRIO field for this command
    } else {
        cmdTable->CFIS.Count15_8 = hintPayload->Count15_8;
    }

    if ((command == IDE_COMMAND_NCQ_NON_DATA) && (subCommand == IDE_NCQ_NON_DATA_HYBRID_CHANGE_BY_LBA_RANGE)) {
        cmdTable->CFIS.LBA7_0 = hintPayload->LBA7_0;
        cmdTable->CFIS.LBA15_8 = hintPayload->LBA15_8;
        cmdTable->CFIS.LBA23_16 = hintPayload->LBA23_16;
        cmdTable->CFIS.LBA31_24 = hintPayload->LBA31_24;
        cmdTable->CFIS.LBA39_32 = hintPayload->LBA39_32;
        cmdTable->CFIS.LBA47_40 = hintPayload->LBA47_40;
    } else if ((command == IDE_COMMAND_NCQ_NON_DATA) && (subCommand == IDE_NCQ_NON_DATA_HYBRID_CONTROL)) {
        cmdTable->CFIS.LBA7_0 = hintPayload->LBA7_0;
        cmdTable->CFIS.LBA15_8 = hintPayload->LBA15_8;
        cmdTable->CFIS.LBA23_16 = 0;
        cmdTable->CFIS.LBA31_24 = 0;
        cmdTable->CFIS.LBA39_32 = 0;
        cmdTable->CFIS.LBA47_40 = 0;
    } else {
        cmdTable->CFIS.LBA7_0 = 0;
        cmdTable->CFIS.LBA15_8 = 0;
        cmdTable->CFIS.LBA23_16 = 0;
        cmdTable->CFIS.LBA31_24 = 0;
        cmdTable->CFIS.LBA39_32 = 0;
        cmdTable->CFIS.LBA47_40 = 0;
    ...
    if ((command == IDE_COMMAND_SEND_FPDMA_QUEUED) && (subCommand == IDE_NCQ_SEND_HYBRID_EVICT)) {
        cmdTable->CFIS.Auxiliary7_0 = (hintPayload->Auxiliary7_0 & 0x01);
        cmdTable->CFIS.Auxiliary23_16 = 0;
    } else {
        cmdTable->CFIS.Auxiliary7_0 = 0;
        cmdTable->CFIS.Auxiliary23_16 = hintPayload->Auxiliary23_16;
    }
...
*/
{
  ...
  //2. Program the CFIS in the CommandTable (allocated in srbExtension)
    ...
    } else if ( IsAtaHintPayload(srbExtension->AtaFunction) ) {
        HintPayloadtoATA_CFIS(ChannelExtension, slotContent);
