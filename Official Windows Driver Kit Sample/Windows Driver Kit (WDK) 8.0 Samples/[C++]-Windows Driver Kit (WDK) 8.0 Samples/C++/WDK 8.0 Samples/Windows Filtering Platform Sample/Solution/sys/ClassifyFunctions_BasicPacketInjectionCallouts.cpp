...
#pragma warning(pop)
   ...
   *ppInjectionData = 0;
   ...
   /// Handle if checksum Offload occurred, if its loopback, or if the packet was IPsec secured
   if(checksumInfo.Receive.NdisPacketIpChecksumSucceeded ||
      ...
      flags & FWP_CONDITION_FLAG_IS_LOOPBACK ||
      pCompletionData->pInjectionData->isIPsecSecured)
   {
   ...
   if(pRemoteAddress)
   {
      if((pRemoteAddress->type == FWP_UINT32 &&
         RtlCompareMemory(&(pRemoteAddress->uint32),
                          IPV4_LOOPBACK_ADDRESS,
                          IPV4_ADDRESS_SIZE)) ||
         (pRemoteAddress->type == FWP_BYTE_ARRAY16_TYPE &&
