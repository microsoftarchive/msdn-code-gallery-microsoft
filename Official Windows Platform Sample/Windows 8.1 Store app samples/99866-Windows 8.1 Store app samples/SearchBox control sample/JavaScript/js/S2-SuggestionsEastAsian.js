...
(function () {
    ...
    function appendSuggestion(textToMatch, suggestionCollection) {
        for (var i = 0, suggestionLength = suggestionList.length; i < suggestionLength; i++) {
            if (suggestionList[i].substr(0, textToMatch.length).toLowerCase() === textToMatch) {
    ...
    function suggestionsRequestedHandler(eventObject) {
        ...
        if (queryText.length > 0) {
            // first try matching against the query text shown in the SearchBox
            appendSuggestion(queryText, suggestionCollection);

            // now try matching against the query alternatives
            for (var j = 0, len = queryAlternatives.length; j < len; j++) {
                ...
                var alternative = queryAlternatives[j].toLowerCase();
                appendSuggestion(alternative, suggestionCollection);
            }
        }
    }
