...
void OperatorNotification::Run(IBackgroundTaskInstance^ taskInstance)
{
    ...
    else if ((notificationEventData->NotificationType == NetworkOperatorEventMessageType::TetheringNumberOfClientsChanged) ||
             (notificationEventData->NotificationType == NetworkOperatorEventMessageType::TetheringOperationalStateChanged))
    {
        XmlDocument^ toastXml = ToastNotificationManager::GetTemplateContent(ToastTemplateType::ToastText02);
        XmlNodeList^ stringElements = toastXml->GetElementsByTagName("text");
    ...
    settings->Values->Insert(key + "_type", notificationEventData->NotificationType.ToString());
}
