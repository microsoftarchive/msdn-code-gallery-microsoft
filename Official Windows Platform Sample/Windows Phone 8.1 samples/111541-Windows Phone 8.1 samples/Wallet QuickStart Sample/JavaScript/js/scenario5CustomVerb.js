...
(function () {
    "use strict";
    var page = WinJS.UI.Pages.define("/html/scenario5CustomVerb.html", {
            ready: function (element, options) {
                document.getElementById("addCustomVerb").addEventListener("click", addCustomVerb);
    ...
    function addCustomVerb() {
        if (walletItem) {
            walletItem.verbs.insert("addfunds", new Windows.ApplicationModel.Wallet.WalletVerb("Add Funds"));
            wallet.updateAsync(walletItem).done(function () {
                WinJS.log && WinJS.log("Custom verb added.", "sample", "status");
            }, function (error) {
                WinJS.log && WinJS.log("Error: " + error, "sample", "error");
            });
        }
