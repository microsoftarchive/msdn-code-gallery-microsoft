...
namespace VoiceCommandsQuickstart
{
	...
	public sealed partial class MessagePage : Page
	{
		...
		protected override async void OnNavigatedTo(NavigationEventArgs e)
		{
			...
			{
				SuspensionManager.SessionState["MessagePage"] = e.Parameter.ToString();
			}

			string[] parameters = SuspensionManager.SessionState["MessagePage"].ToString().Split('|');

			if (parameters.Length > 2)
			{
				contactTextBlock.Text = parameters[1];
				msgTextTextBlock.Text = parameters[2];
				...
				{
					string feedback = string.Format("VoiceCommandsQuickstart has launched and is messaging {0} with the message {1}", contactTextBlock.Text, msgTextTextBlock.Text);
