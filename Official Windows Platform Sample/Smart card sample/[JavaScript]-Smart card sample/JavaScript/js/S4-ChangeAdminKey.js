...
(function () {
    "use strict";
    var page = WinJS.UI.Pages.define("/html/S4-ChangeAdminKey.html", {
        ready: function (element, options) {
            document.getElementById("ChangeAdminKey").addEventListener(
                "click",
                ChangeAdminKey_Click,
                false);
        }
    });

    function ChangeAdminKey_Click() {
        if (null === SdkSample.reader) {
            WinJS.log("Use Scenario One to create a TPM virtual smart card.",
                      "sample",
                      "error");
            return;
        }

        var newAdminKey = Windows.Security.Cryptography.CryptographicBuffer
        ...
        function (challengeContext) {
            var response = SdkSample.calculateChallengeResponse(
                ...
                SdkSample.adminKey);
            return challengeContext.changeAdministrativeKeyAsync(response,
                                                                 newAdminKey);
        }).done(
        function () {
            SdkSample.adminKey = newAdminKey;
            WinJS.log("Smart card change admin key operation completed.",
                      "sample",
                      "status");
