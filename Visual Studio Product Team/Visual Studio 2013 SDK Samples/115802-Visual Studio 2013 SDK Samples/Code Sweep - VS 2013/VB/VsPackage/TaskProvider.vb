        ...
        Public Sub AddResult(ByVal result As IScanResult, ByVal projectFile As String) Implements ITaskProvider.AddResult
            ...
            If result.Scanned Then
                For Each hit As IScanHit In result.Results
                    If Not String.IsNullOrEmpty(hit.Warning) Then
                        ' See if we've warned about this term before; if so, don't warn again.
                        If _termsWithDuplicateWarning.Find(Function(item) String.Compare(item, hit.Term.Text, StringComparison.OrdinalIgnoreCase) = 0) Is Nothing Then
                            _tasks.Add(New Task(hit.Term.Text, hit.Term.Severity, hit.Term.Class, hit.Warning, String.Empty, String.Empty, -1, -1, String.Empty, String.Empty, Me, _serviceProvider))
                            _termsWithDuplicateWarning.Add(hit.Term.Text)
                        End If
                    End If
                    _tasks.Add(New Task(hit.Term.Text, hit.Term.Severity, hit.Term.Class, hit.Term.Comment, hit.Term.RecommendedTerm, fullPath, hit.Line, hit.Column, projectFile, hit.LineText, Me, _serviceProvider))
                Next
            Else
                _tasks.Add(New Task(String.Empty, 1, String.Empty, String.Format(CultureInfo.CurrentUICulture, Resources.FileNotScannedError, fullPath), "", fullPath, -1, -1, projectFile, "", Me, _serviceProvider))
            End If
            Refresh()
        End Sub
...
#End Region ' ITaskProvider Members
...
#End Region ' IVsTaskProvider Members
...
#End Region ' IVsTaskProvider3 Members
...
#End Region ' IVsSolutionEvents Members
...
#Region "Private Members"
        ...
        Private ReadOnly _termsWithDuplicateWarning As New List(Of String)()
        ...
        Private Sub SetCommandHandlers()
            ...
            Dim showIgnoredCommand As New OleMenuCommand(New EventHandler(AddressOf ToggleShowIgnoredInstances), showIgnoredID)
            mcs.AddCommand(showIgnoredCommand)
        End Sub

        Private Function GetSelectedTasks() As List(Of Task)
        ...
        Private Sub QueryIgnore(ByVal sender As Object, ByVal e As EventArgs)
            ...
            If IsActiveProvider Then
                Dim anyNotIgnored As Boolean = GetSelectedTasks().Any(Function(t) Not t.Ignored)
        ...
        Private Sub QueryDontIgnore(ByVal sender As Object, ByVal e As EventArgs)
            ...
            If IsActiveProvider Then
                Dim anyIgnored As Boolean = GetSelectedTasks().Any(Function(t) t.Ignored)
        ...
        Private Sub SetSelectedItemsIgnoreState(ByVal ignore As Boolean)
            For Each item As Task In GetSelectedTasks()
                item.Ignored = ignore
            Next
            Refresh()
        End Sub

        Private Sub ToggleShowIgnoredInstances(ByVal sender As Object, ByVal e As EventArgs)
