...
#End Region ' Private Members
        ...
        Private _currentLineText As String = String.Empty
        ...
        Private Shared Function CreateSortedExclusionIndex(ByVal sortedTermIndex As List(Of MatchableTerm)) As List(Of MatchableExclusion)
            ...
            index.Sort(Function(x, y) CompareStringsWithLongestFirst(x.Text, y.Text))
        ...
        Private Shared Function CreateSortedTermIndex(ByVal termTables As IEnumerable(Of ITermTable)) As List(Of MatchableTerm)
            ...
            index.Sort(AddressOf CompareMatchableTerms)
        ...
        Private Shared Function CompareMatchableTerms(x As MatchableTerm, y As MatchableTerm) As Integer
        ...
        Private Function GetDuplicatesOf(ByVal sourceTerm As MatchableTerm) As IEnumerable(Of MatchableTerm)
            Return _termIndex.Where(Function(possibleDup) String.Compare(sourceTerm.Term.Text, possibleDup.Term.Text, StringComparison.OrdinalIgnoreCase) = 0)
        End Function
        ...
        Private ReadOnly Property PartiallyMatchedTerms() As IEnumerable(Of TermMatch)
            Get
                Return _partialMatches.OfType(Of TermMatch)()
            End Get
        End Property

        Private ReadOnly Property PartiallyMatchedExclusions() As IEnumerable(Of ExclusionMatch)
            Get
                Return _partialMatches.OfType(Of ExclusionMatch)()
            End Get
        End Property
        ...
        Private Function TermsStartingWith(ByVal c As Char) As IEnumerable(Of MatchableTerm)
            Return BinarySearch(_termIndex, Function(term) AscW(Char.ToLowerInvariant(term.Term.Text(0))) - AscW(Char.ToLowerInvariant(c)))
        End Function

        Private Function ExclusionsStartingWith(ByVal c As Char) As IEnumerable(Of MatchableExclusion)
            Return BinarySearch(_exclusionIndex, Function(exclusion) AscW(Char.ToLowerInvariant(exclusion.Text(0))) - AscW(Char.ToLowerInvariant(c)))
        End Function
