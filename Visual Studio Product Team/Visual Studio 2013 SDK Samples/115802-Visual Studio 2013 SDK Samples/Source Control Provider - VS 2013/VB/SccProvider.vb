...
Namespace Microsoft.Samples.VisualStudio.SourceControlIntegration.SccProvider
    ...
    <MsVsShell.DefaultRegistryRoot("Software\Microsoft\VisualStudio\12.0Exp"), _
     ...
     MsVsShell.InstalledProductRegistration("#100", "#101", "1.0", IconResourceID:=CommandId.iiconProductIcon), _
     MsVsShell.ProvideOptionPageAttribute(GetType(SccProviderOptions), "Source Control", "Sample Options Page", 106, 107, False), _
     ProvideToolsOptionsPageVisibility("Source Control", "Sample Options Page", GuidStrings.GuidSccProvider), _
     ...
     MsVsShell.ProvideService(GetType(SccProviderService), ServiceName:="Source Control Sample Provider Service"), _
     ProvideSourceControlProvider("Managed Source Control Sample Provider", "#100"), _
     MsVsShell.ProvideAutoLoad(GuidStrings.GuidSccProvider), _
     ProvideSolutionProps(Microsoft.Samples.VisualStudio.SourceControlIntegration.SccProvider.SccProvider._strSolutionPersistanceKey), Guid(GuidStrings.GuidSccProviderPkg)> _
    Public NotInheritable Class SccProvider ' We'll write properties in the solution file to track when solution is controlled; the interface needs to be implemented by the package object
        ...
        Private Const _showDebugTraceMessages As Boolean = False
...
#Region "Source Control Command Enabling"
        ...
        Public Shadows Function QueryStatus(ByRef guidCmdGroup As Guid, ByVal cCmds As UInteger, ByVal prgCmds As OLECMD(), ByVal pCmdText As System.IntPtr) As Integer Implements IOleCommandTarget.QueryStatus
...
#Region "Source Control Utility Functions"
        ...
        Private Sub DebugWalkingNode(ByVal pHier As IVsHierarchy, ByVal itemid As UInteger)
            If SccProvider._showDebugTraceMessages Then
                ...
                If pHier.GetProperty(itemid, CInt(Fix(__VSHPROPID.VSHPROPID_Name)), [property]) = VSConstants.S_OK Then
                    Debug.WriteLine(String.Format(CultureInfo.CurrentUICulture, "Walking hierarchy node: {0}", CType([property], String)))
                End If
            End If
        End Sub
