...
HRESULT SymbolEngine::AddressForVA(ULONGLONG va, _Out_ DWORD *pISect, _Out_ DWORD *pOffset)
{
    return m_pDiaSession->addressForVA(va, pISect, pOffset);
}
...
HRESULT SymbolEngine::FindSourceForAddr(BSTR bstrModuleName, DWORD dwModuleBase, DWORD dwRvaIp, BSTR* pbstrDocumentName, BSTR* pbstrFunctionName, DWORD* pdwOffset, DWORD* pdwArgs, DWORD* pdwLocals)
{
	...
	{		
		...
		*pdwLocals = 0;
		...
		m_pDiaSession->findLinesByAddr(dwSection, dwAddrOffset, (DWORD)ullLen, &pEnumLines);
